...
def main():
    ...
    sig_idx= [0,1,2,3,4] #which nodes activation should be sigmoidal
    
    #relu_idx= [1,1] #we want 2 and 3 to ramp up linearly, so units 2 and 3 are relu (index 1 and 2)
    
    steps = 5000
    
    weights = np.array([[0,    -2,      0,      -2,     0], #1->1  2->1, 3->1, 4->1, 5->1
                        [-2,    0,      -2,      0,     0], #1->2, 2->2, 3->2, 4->2, 5->2
                        [.3,    0,      2.2,    0,      0], #1->3, 2->3, 3->3, 4->3, 5->3
                        [0,     1.23,   0,      2.087,  0], #1->4, 2->4, 3->4, 4->4, 5->4
                        [0,     0,      0,      1.3,   1]])#1->5, 2->5, 3->5, 4->5, 5->5
    ...
    v = np.array([[0, 0, 0, 0, 0]]).T              # values for each neuron
    v_hist = np.array([[0, 0, 0, 0, 0]]).T       # a list of columns of activation values at each time stamp
    piecewise_vals = [0]
    v2_v1_diff = [0]
    input_vals = np.array([]).T
    input_vals_delta = np.array([]).T
    
    l = np.array([[4, 4, 4, 4, 12]]).T                  #steepness of activation fxn
    bias = np.array([[1, 1, 1, 1, 1]]).T 
    ...
    for i in range (steps):
        ...
        dv = tau * ((-v + activations) * dt + noise * np.sqrt(dt) * np.random.normal(0,1, (5,1))) # add noise using np.random
        v = v + dv
    ...
    plt.figure()
    plt.plot(v2_v1_diff)
    plt.legend("Difference between unit 2 and unit 1")
    plt.ylabel("input")
    ...
    plt.legend(["v1","v2","v3","v4", "v5 - The \"sure\" v4", "piecewise"], loc=0)
